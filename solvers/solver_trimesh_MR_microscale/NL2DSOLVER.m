function [WEFF, DISPTD_FINAL] = NL2DSOLVER(FIXEDNODES, p, PROP, NE, GDOF, ND, g, PHASES, sb, sc, Ae, DOFe, ISPARSE, JSPARSE, KSPARSE)
...
while (FLAG10 == 1)
    ...
    DISPDD = zeros(GDOF, 1);
        ...
        while (FLAG20 == 1)
            ...
            [X, Y] = mex_HYPER2D(PROP,NE,PHASES,sb,sc,Ae,GDOF,DOFe,DISPTD);
            GKF = sparse(ISPARSE, JSPARSE, X, GDOF, GDOF);
            FORCE = sparse(KSPARSE, ones(6*NE, 1), Y, GDOF, 1);
...
%WEFF = EFFSEF_CPP(DISPTD, NE, PROP, PHASES, sb, sc, Ae, DOFe);
WEFF = mex_EFFSEF(DISPTD, NE, PROP, PHASES, sb, sc, Ae, DOFe);
DISPTD_FINAL = DISPTD;
